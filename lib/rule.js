	...
	return async function transform(source) {
		...
		for (let i = arr.length-1; i >= 0; i--) {
			...
			if (typeof loader === 'string') {
				...
				// implementers to write a loader that can be used both with 
				// `test`, or simply as a string!
				let module = await import(loader);
				loader = module.rawTransform || module.default;

			}

			// TODO: If we want compatibility wiht exisitng webpack loaders,we 
			// hav to mock the webpack context here somehow. We don't know 
			// enough about webpack to do this now though, PRs are welcome if 
			// someone can use it.
			if (options.compat) {
				let ctx = {
					getOptions: () => options,
				};
				output = await loader.call(ctx, output);
			} else {
				output = await loader(output, { options });
			}
